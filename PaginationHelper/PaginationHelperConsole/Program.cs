using PaginationHelperConsole;

PaginationHelper<char> helper = new(['a', 'b', 'c', 'd', 'e', 'f'], 4);
...
namespace PaginationHelperConsole
{
    public class PaginationHelper<T>(IList<T> collection, int itemsPerPage)
    {
        ...
        {
            return itemIndex < 0 || itemIndex >= _collection.Count ? -1 : itemIndex / _itemsPerPage;
        }
    }
}
